...
- 按 Android 系统版本选择 SurfaceComposer 兼容符号，覆盖 Android 8 到 17（8、9、10、11、12~13、14~16、17 各一套）。
- 默认被动监听 `/dev/input/event*`，不加 `EVIOCGRAB` 独占设备，底层应用仍能收到原始触摸事件；切到注入模式时通过 `/dev/uinput` 创建虚拟触摸设备。
...
- 示例 UI 保留渲染接口与 GUI 版本信息，提供无图标的穿透开关、独立的图片/GIF 绘制控件，以及 60~185 FPS 的 GUI 帧率调节。
- 过录制开关打开时会隐藏已建立的镜像层并停止新建，避免画面被投到副屏 layer stack 上。
- ImGui 窗口不绘制边框，叠加层只保留窗口自身内容。
...
4. 每帧执行 `drawBegin()`、`NewFrame(true)`、`Layout_tick_UI()`、`EndFrame()`；`drawBegin()` 返回 false（Surface 重建失败）时退出主循环。
5. 退出时销毁纹理、ImGui context、Vulkan 资源和 Surface，并调用 `ANativeWindowCreator::Cleanup()` 兜底清理派生 Surface。
...
第二个参数为 `true` 时只被动监听物理触摸、不使用 `EVIOCGRAB` 独占设备；置 `false` 则切换到 `/dev/uinput` 注入模式。旋转变化由 `Touch::setOrientation(displayInfo.orientation)` 同步到触摸坐标转换。
